...
const ALL = [
  ...
  ['f-harbour-road', 25, 0, 6, 0.02, 0.06],
  ...
  ['m-tower', -28, 0, -2, -1.35, 0.35],
  ['n-warehouse-out', 25, 0, -8, 0.0, 0.12],
  ['o-customs-in', 25, 0.15, 25, 0.0, 0.0],
  ['p-plaza-wide', -11, 0, 30, 0.55, 0.10],
];
const SHOTS = HALF === 1 ? ALL.slice(0, 8) : HALF === 2 ? ALL.slice(8) : ALL;
...
await page.evaluate(() => window.__GAME__.startMatch({ mode: 'tdm', botCount: 0, difficulty: 'recruit' }));
...
for (const [name, x, y, z, yaw, pitch] of SHOTS) {
  ...
  await page.evaluate((s) => {
    ...
    if (g.state !== 'playing') g.startMatch({ mode: 'tdm', botCount: 0, difficulty: 'recruit' });
    const p = g.player;
    if (!p) return;
  ...
  } catch (e) { console.log('[look] FAILED', name, String(e).slice(0, 200)); }
}
