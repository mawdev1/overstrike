...
function makeShared(ctx) {
  ...
  return { white, pink, hasPan: typeof ctx.createStereoPanner === 'function' };
}
...
async function renderBatch(specs, channels, sampleRate) {
  ...
  const rendered = await Promise.all(
    chunks.map((c) => renderChunk(c, channels, sampleRate, OAC).catch((err) => {
...
function buildIR(ctx, def, seed) {
  ...
  for (let c = 0; c < 2; c++) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      let env = Math.exp(-t * invTau);
      if (build > 0 && t < build) env *= t / build;
      d[i] = (r() * 2 - 1) * env;
    }
