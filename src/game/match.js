...
// BotManager keeps its own 3.5 s fallback timer. Coming in just under it means the
// scored spawn below wins the race in practice, and the fallback stays as a safety net.
...
export class Match {
  ...
  _onShot(p) {
    ...
    if (p?.shooter) this._protect.delete(p.shooter.id);
  ...
  _queueRespawn(entity, killer) {
    ...
    const delay = isPlayer ? PLAYER_RESPAWN : BOT_RESPAWN;
    this._respawns.push({
      entity,
