...
/**
 * Release a mode's shared `_gfx` bundle: detach the group, dispose the one shared
 * geometry and every material on it, and drop the handle so a later `init()` rebuilds.
 * Modes are singletons that outlive any single Match, so this belongs to teardown
 * (Match.dispose) rather than to `cleanup()`, which runs on every match end.
 */
function disposeGfx(mode) {
  const gfx = mode._gfx;
  if (!gfx) return;
  if (gfx.group?.parent) gfx.group.parent.remove(gfx.group);
  for (const m of gfx.meshes || []) {
    m.material?.dispose?.();
    m.dispose?.();          // InstancedMesh owns an instance buffer; Mesh does not.
  }
  gfx.geo?.dispose?.();
  mode._gfx = null;
}
...
const DOMINATION = {
  ...
  cleanup(match) {
    // Hide rather than detach: the rig must stay in `engine.scene` so the boot prewarm
    // (and any later `compile()`) keeps seeing it. Hidden costs nothing to render.
    const gfx = match.modeState.gfx || this._gfx;
    if (gfx?.group) gfx.group.visible = false;
  },
  ...
  /**
   * Build the zone rig and park it in the scene, hidden.
   *
   * Called from `Match.init()` at BOOT, before `Game._prewarmShaders()` runs. That
   * matters: the prewarm force-shows everything in `engine.scene` and compiles it, so a
   * rig that exists by then costs zero shader links later. Built lazily inside `init()`
   * instead — as it used to be — its two programs linked the first time a zone was
   * actually drawn, which is a hard stall inside a gameplay frame of the first
   * DOMINATION match of the session. (Measured: 2 programs for DOMINATION, 1 for
   * KILL CONFIRMED, all of them after match start.)
   */
  prewarm(match) { this._ensureGfx(match); },
  ...
  _ensureGfx(match) {
    ...
    if (!gfx) {
      ...
      // Hidden until a DOMINATION match actually starts — the prewarm sweep shows it
      // anyway, and every other mode must not pay three draw calls for it.
      group.visible = false;
      gfx = this._gfx = { geo, group, meshes, lastOwner: [-2, -2, -2] };
      match.game.scene?.add(group);
    }
    return gfx;
  },
  ...
  _buildGfx(match, zones) {
    const st = match.modeState;
    const gfx = this._ensureGfx(match);
    for (let i = 0; i < 3; i++) {
...
const KILLCONFIRMED = {
  ...
  /** See DOMINATION.prewarm — built at boot so its program links there, not mid-match. */
  prewarm(match) { this._ensureGfx(match); },
