...
function acquireSlot(game, team) {
  ensureTeam(game, team);
  const free = RIG.free[team];
  if (free.length) return free.pop();
  if (RIG.next[team] >= RIG_CAPACITY) {
    console.warn('[botModel] rig capacity exhausted; extra bots will be invisible');
    return -1;
  }
  return RIG.next[team]++;
}

function releaseSlot(team, slot) {
  if (slot < 0 || !RIG.meshes[team]) return;
  const meshes = RIG.meshes[team];
  for (let i = 0; i < BONE_COUNT; i++) {
    meshes[i].setMatrixAt(slot, _zeroMatrix);
    meshes[i].instanceMatrix.needsUpdate = true;
  }
  RIG.free[team].push(slot);
}
...
export class BotModel {
  ...
  constructor(game, team) {
    ...
    this.slot = acquireSlot(game, this.team);
    this.visible = true;
    this.disposed = false;
    RIG.live++;
  ...
  _blit() {
    ...
    for (let i = 0; i < BONE_COUNT; i++) {
      meshes[i].setMatrixAt(slot, list[i].matrixWorld);
      meshes[i].instanceMatrix.needsUpdate = true;
    }
  }
  ...
  dispose() {
    ...
    this.disposed = true;
    releaseSlot(this.team, this.slot);
    this.slot = -1;
