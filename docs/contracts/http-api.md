...
| DELETE | `/v1/rooms/:id` | A | Owner-only. `409 ROOM_NOT_EMPTY` unless the owner is the only member. A live match does **not** refuse the delete: it is ended first through the ordinary terminal saga (canonical `status:"aborted"` result, then `POST /control/release` on the authority and the registry reservation released), and only then is the room destroyed. Since the owner must be its only member, this can only ever end a match that is theirs alone. Closing a room with other members still in it requires them to leave first — no live-connected member is ever force-disconnected by another party's action anywhere in this contract, and this endpoint does not introduce the first one |
