...
|---|---|---|---|
| 1 | [`http-api.md`](http-api.md) | REVIEW | Platform REST surface for P1–P4 |
| 2 | [`errors.md`](errors.md) | REVIEW | Error envelope and the closed error-code enumeration |
| 3 | [`auth.md`](auth.md) | REVIEW | Tokens, sessions, revocation, recovery |
| 4 | [`realtime-lobby.md`](realtime-lobby.md) | REVIEW | Lobby WebSocket: presence, roster, teams, ready, launch |
| 5 | [`wire-protocol.md`](wire-protocol.md) | REVIEW | The binary match protocol, as it exists and as it must change |
| 6 | [`net-facade.md`](net-facade.md) | REVIEW | The only surface Codex uses to reach netcode |
| 7 | [`match-result.md`](match-result.md) | REVIEW | Canonical stat definitions and the immutable result record |
| 8 | [`event-envelope.md`](event-envelope.md) | REVIEW | Canonical platform event envelope and catalogue |
| 9 | [`map-data.md`](map-data.md) | REVIEW | What `level.js` must export for the server to consume |
| 10 | [`bomb-rules.md`](bomb-rules.md) | REVIEW | The Bomb ruleset |
| 11 | [`db-schema.md`](db-schema.md) | REVIEW | Initial PostgreSQL schema |
| 12 | [`telemetry.md`](telemetry.md) | REVIEW | Client and server telemetry, privacy and retention classes |
| 13 | [`feature-flags.md`](feature-flags.md) | REVIEW | Flag naming, evaluation, defaults, kill switches |

All six P0.3 decisions are resolved — see [`../decisions/P0-decisions.md`](../decisions/P0-decisions.md).
No contract has a `PENDING DECISION` section remaining. The single blocker to `FROZEN` is
`REQ-CX-001`, the Codex sufficiency sign-off.
