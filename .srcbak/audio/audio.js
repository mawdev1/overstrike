...
export class AudioEngine {
  ...
  play(name, opts) {
    ...
    if (!this._reserveVoice(name, priority, now)) return null;
   ...
   *
   * @param {string} name @param {number} priority @param {number} now
   */
  _reserveVoice(name, priority, now) {
    this._pruneVoices();
    ...
    for (let i = 0; i < this.voices.length; i++) {
      ...
      if (now - v.startedAt < MIN_STEAL_AGE) continue;   // too young to interrupt
